from tkinter import *
import math

# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 20
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 25
reps = 1
timer = None


# ---------------------------- TIMER RESET ------------------------------- #
def reset():
    global reps
    window.after_cancel(timer)
    title_text.config(text="Timer", fg=GREEN)
    canvas.itemconfig(timer_text, text="00:00", fill="white")
    check_mark.config(text="")
    reps = 1


# ---------------------------- TIMER MECHANISM ------------------------------- #
def start_timer():
    global reps
    window.deiconify()
    if reps % 8 == 0:
        reps = 1
        count_down(LONG_BREAK_MIN * 60)
        add_marks()
        title_text.config(text="Break", fg=RED)
        canvas.itemconfig(timer_text, fill=RED)
    elif reps % 2 == 0:
        reps += 1
        count_down(SHORT_BREAK_MIN * 60)
        add_marks()
        title_text.config(text="Break", fg=PINK)
        canvas.itemconfig(timer_text, fill=PINK)
    else:
        if check_mark.cget("text") == "✔✔✔✔":
            check_mark.config(text="")
        reps += 1
        count_down(WORK_MIN * 60)
        title_text.config(text="Work!", fg=GREEN)
        canvas.itemconfig(timer_text, fill=GREEN)


def add_marks():
    check_mark.config(text=check_mark.cget("text") + "✔")


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #
def count_down(count):
    count_min = math.floor(count / 60)
    count_sec = (count % 60)

    canvas.itemconfig(timer_text, text=f"{count_min}:%02d" % count_sec)
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()


# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)
window.attributes('-topmost', True)

title_text = Label(text="Timer", bg=YELLOW, font=(FONT_NAME, 50, "bold"), fg=GREEN)
title_text.grid(column=2, row=0)

canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=FALSE)
tomato_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT_NAME, 25, "bold"))
canvas.grid(column=2, row=1)

start_button = Button(text="Start", highlightthickness=FALSE, command=start_timer)
start_button.grid(column=1, row=2)
reset_button = Button(text="Reset", highlightthickness=FALSE, command=reset)
reset_button.grid(column=3, row=2)

check_mark = Label(bg=YELLOW, font=(FONT_NAME, 10, "bold"), fg=GREEN)
check_mark.grid(column=2, row=3)

window.mainloop()
